...
export let selected = {type: -1, index: -1, held: false, shiftHeld: false};
let renderer = null;
...
export function initRenderer() {
    ...
    renderer = new Renderer([new CanvasBackend(canvas), new SvgBackend(svg)]);
}
...
function drawGraph(renderer) {
    ...
    //Draw connections
    for(let i in graphstuff.links) {
        if(selected.type == 1 && selected.index == i) {
            renderer.setColor("blue");
        }

        let link_pos = graphstuff.getLinkPositions(i);
        let transition = graphstuff.graph.transitions[i];
        let link = graphstuff.links[i];
        let text = graphstuff.getLinkText(i);
        drawLink(renderer, link_pos[0], link_pos[1], link.curve, text, transition.start == transition.end, transition.end >= 0, link.flip);
    ...
    //Draw nodes
    for(let i in graphstuff.positions) {
        if(selected.type == 0 && selected.index == i) {
            renderer.setColor("blue");
        }
        
        let pos = graphstuff.positions[i];
        let node = graphstuff.graph.states[i];
        drawNode(renderer, pos, node.name, node.accept, graphstuff.graph.start == i);
...
export function clearSelection() {
    selected = {type: -1, index: -1, held: false, shiftHeld: false};
}
...
function onKeyDown(event) {
    ...
    if(event.key == "Delete" || event.key == "Backspace") {
        if(selected.type == 0) {
            graphstuff.removeState(selected.index);
        } else {
            graphstuff.removeTransition(selected.index);
        }
...
function onDoubleClick(event) {
    ...
    graphstuff.addState("name", false, [event.offsetX, event.offsetY]);
    selected.type = 0;
    selected.index = graphstuff.positions.length - 1;
    selected.held = false;
    refreshEditor();
    props.updateProperties();
}
...
function onMouseDown(event) {
    ...
    for(let pos of graphstuff.positions) {
        if(Math.pow(event.offsetX - pos[0], 2) + Math.pow(event.offsetY - pos[1], 2) < 400) {
            selected.type = 0;
            selected.index = graphstuff.positions.indexOf(pos);
            selected.held = true;
            break;
        }
    }
        ...
        for(let i in graphstuff.links) {
            ...
            if(transition.start == transition.end) {
                ...
                if(curve == 0) {
                    ...
                    let normal = [diff[1], -diff[0]];
                    let r = calcArcRadius(a, [middle[0] + normal[0] * curve, middle[1] + normal[1] * curve], b);
                    let center = r[1];
                ...
                if(dist_sq < 100) {
                    ...
                    selected.index = i;
                    selected.held = true;
                    selected.shiftHeld = event.shiftKey && transition.end >= 0;
                    break;
                }
            }
        }
    ...
    if(event.shiftKey && selected.type == 0) {
        graphstuff.addTransition(selected.index, selected.index, false, [event.offsetX, event.offsetY], 0, false);
        selected.type = 1;
        selected.index = graphstuff.links.length - 1;
        selected.held = true;
        selected.shiftHeld = false;
    }
...
function onMouseMove(event) {
    if(selected.type == 0 && selected.held) {
        graphstuff.setPosition(selected.index, [Math.floor(event.offsetX / 5) * 5, Math.floor(event.offsetY / 5) * 5]);
    }
    if(selected.type == 1 && selected.held) {
        let link_pos = graphstuff.getLinkPositions(selected.index);
        let transition = graphstuff.getTransition(selected.index);
        if(selected.shiftHeld && transition.transition.start != transition.transition.end) {
            let a = link_pos[0];
            let b = link_pos[1];
            let middle = [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2];
            ...
            if(diff < 10) {
                transition.link.flip = dot < 0;
                diff = 0;
            }

            transition.link.curve = diff * Math.sign(dot);
        } else if(selected.shiftHeld) {
            //Repurpose curve offset as angle for circular link
            let center = link_pos[0];
            transition.link.curve = Math.atan2(event.offsetY - center[1], event.offsetX - center[0]);
        } else {
            transition.link.end = [Math.floor(event.offsetX / 5) * 5, Math.floor(event.offsetY / 5) * 5];
            transition.link.curve = 0;
            transition.transition.end = -1;
            for(let i in graphstuff.positions) {
                let pos = graphstuff.positions[i];
                if(Math.pow(event.offsetX - pos[0], 2) + Math.pow(event.offsetY - pos[1], 2) < 400) {
                    transition.transition.end = Number(i);
                    break;
                }
            }
        }
